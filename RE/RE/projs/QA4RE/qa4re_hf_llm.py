...
def call_gpt_df(args, input_ready_df,output_dir):
    predictions = []
    model_generations = []
    time_list = []
    ...
    for i, row in tqdm(input_ready_df.iterrows()):
        ...
        generation=completions_with_backoff(model=args.model, messages=[{"role": "user", "content": final_input_prompt}], temperature=0, top_p=0)#[0]
        #print(generation)
        match_g=pattern.match(generation)
        if match_g:
            generation=match_g.group()
        else:
            print("random choice "+generation+"at "+str(i)+"-th iter")
            generation=str(random.randint(1,50))
...
def call_hf_llm_df(args, input_ready_df, output_dir):
    ...
    #model=torch.nn.parallel.DistributedDataParallel(model,device_ids=[0,1,2],output_device=device_id).cuda()
    for i, row in tqdm(input_ready_df.iterrows()):
        time_start = time.time()
        final_input_prompt = row['final_input_prompts']
        all_choices = row['all_choices']
...
def run_qa4re(args):
    ...
    if args.model.find("gpt")<0:
        output_df = call_hf_llm_df(args, input_ready_df, output_dir)
    else:
        output_df = call_gpt_df(args, input_ready_df,output_dir)
    ...
    print("input_ready_df.shape",input_ready_df.shape,subset_test.shape)
