...
from transformers import GPT2Tokenizer, AutoTokenizer, LlamaForCausalLM#, BaichuanTokenizer
...
def fill_prompt_format_re_df(args, df, prompt_config, sent_col='masked_sents'):
    ...
    for i, row in df.iterrows():
        ...
        if args.dataset!="DuIE":
            sent = row[sent_col].replace('ENT1', f"[{ent1}]").replace('ENT2', f"[{ent2}]")
        else:
            sent = row[sent_col].replace('主体', f"[{ent1}]").replace('客体', f"[{ent2}]")
        ...
        if args.type_constrained:
            ...
            valid_relations= list(args.LABEL_TEMPLATES.keys())
            try:
                valid_relations.remove(args.NOTA_RELATION)
            except:
                pass
            valid_relations.append(args.NOTA_RELATION)
        #start_chr = 'A'
        start_chr="1"
        index2rel = {}
...
def build_final_input(df, params, engine, tokenizer):
    ...
    if engine in GPT_MODEL_MAX_TOKEN_DICT:
        ...
        max_allowed_token_num = 8192 #  token num for FLAN-T5
...
def prompt_preparation(args, params, subset_train_df=None, test_df=None):
    ...
    try:
        ...
        tokenizer = AutoTokenizer.from_pretrained("/home/LLMs/Llama2-Chinese-7b-Chat",trust_remote_code=True)
