...
def call_gpt_df(args, input_ready_df,output_dir):
    ...
    for i, row in tqdm(input_ready_df.iterrows()):
        ...
        generation=completions_with_backoff(model=args.model, messages=[{"role": "user", "content": final_input_prompt}], temperature=0, top_p=0)
        #print(generation)
        if generation.find(verbalized_labels[0])>=0:
            generation=verbalized_labels[0]
